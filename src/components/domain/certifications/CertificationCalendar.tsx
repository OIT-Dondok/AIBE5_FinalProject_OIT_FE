...
export function CertificationCalendar({
  ...
  const isSingleDayPeriod = currentPeriod !== null && currentPeriod.start_date === currentPeriod.end_date;
  const initial = useMemo(
    () => parseInitialYM(currentPeriod?.start_date ?? todayYmd),
    [currentPeriod?.start_date, todayYmd],
  );
  const [viewYear, setViewYear] = useState(initial.year);
  const [viewMonth, setViewMonth] = useState(initial.month);
  const [tab, setTab] = useState<Tab>(isSingleDayPeriod ? 'date' : 'period');
  ...
  useEffect(() => {
    void Promise.resolve().then(() => {
      ...
      setTab(isSingleDayPeriod ? 'date' : 'period');
      setSelStart(currentPeriod?.start_date ?? null);
      setSelEnd(currentPeriod?.end_date ?? null);
      setPickingEnd(false);
    });
  }, [currentPeriod?.end_date, currentPeriod?.start_date, initial.month, initial.year, isSingleDayPeriod]);
