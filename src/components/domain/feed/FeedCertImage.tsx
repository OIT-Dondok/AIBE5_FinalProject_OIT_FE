...
/**
 * 크루 인증 카드 썸네일 영역.
 * - imageUrl이 있으면 실제 이미지를 1:1(정사각)로 크롭해 렌더
 * - 없으면 중립 배경 + 이미지 아이콘 placeholder (레이아웃 시프트 방지)
 * 카드 그리드 정렬을 위한 고정 비율 크롭 전용. 원본 비율 확대는 라이트박스가 담당.
 */
export function FeedCertImage({
  ...
  return (
    <div
      className={`relative w-full aspect-square overflow-hidden bg-gradient-to-br from-text-secondary/5 to-text-secondary/15 ${className}`}
    >
